...
(function() {
    ...
    Panda.isOnScreen = function(el, dx, dy) {
        ...
        return y + o.height + ay >= b && y <= b + window.innerHeight + ay && x + o.width + ax >= a && x <= a + window.innerWidth + ax;
    }
    ...
    Panda.isotopes = function(selector, options) {
        if (typeof imagesLoaded === 'function' && $.fn.isotope) {
            ...
            Panda.$(selector).each(function() {
                var $this = $(this),
                    settings = $.extend(true, {}, Panda.defaults.isotope, Panda.parseOptions($this.attr('data-grid-options')), options ? options : {});
                Panda.lazyload($this);
    ...
    Panda.parallax = function(selector, options) {
        if ($.fn.themePluginParallax) {
            Panda.$(selector).each(function() {
                var $this = $(this);
                $this.themePluginParallax($.extend(true, Panda.parseOptions($this.attr('data-parallax-options')), options));
            });
        }
    }
    ...
    Panda.headerToggleSearch = function(selector) {
        ...
        $search.find('input').on('focusin', function(e) {
    ...
    Panda.stickyContent = function(selector, settings) {
                    ...
                    if (options.scrollMode) {
                        if ((showContent && $item.hasClass('fix-top')) || (!showContent && $item.hasClass('fix-bottom'))) {
                            ...
                            if ($item.closest('.page-wrapper').find('.header').hasClass('.header-transparent')) {
                                if (!Panda.$body.hasClass('sidebar-active') && !Panda.$body.hasClass('top-sidebar-active') && !Panda.$body.hasClass('right-sidebar-active')) {
        ...
        var resizeStickyContent = function(e) {
            $stickyContents.removeData('offset-top').removeData('offset-bottom').removeClass('fixed').css('margin', '').css('z-index', '');
            Panda.call(function() {
                initStickyContent();
    ...
    Panda.initTab = function(selector) {
        Panda.$body.on('click', '.tab .nav-link:not(.no-tab-item)', function(e) {
            var $this = $(this);
            e.preventDefault();
    ...
    Panda.popup = function(options, preset) {
        var mpInstance = $.magnificPopup.instance,
            opt = $.extend(true, {}, Panda.defaults.popup, ('undefined' != typeof preset) ? Panda.defaults.popupPresets[preset] : {}, options);
    ...
    Panda.initPopups = function() {
        Panda.$body.on('click', '.single-product a.submit-review-toggle', function(e) {
            e.preventDefault();
            Panda.popup({
                items: {
    ...
    Panda.sidebar = (function() {
        ...
        Sidebar.prototype.init = function(name) {
            ...
            if (self.$sidebar.length) {
                ...
                self.$sidebar.find('.sidebar-toggle').add(name === 'sidebar' ? '.left-sidebar-toggle' : ('.' + name + '-toggle')).on('click', function(e) {
                    self.toggle();
                    $(this).blur();
        ...
        Sidebar.prototype.toggle = function() {
            ...
            if (window.innerWidth >= Panda.minDesktopWidth && self.$sidebar.hasClass('sidebar-fixed')) {
                ...
                if (self.isNavigation) {
                    ...
                    self.$sidebar.find('.sidebar-content').stop().animate({
                        'height': 'toggle',
    ...
    Panda.initProductSingle = (function() {
        ...
        var thumbsRefresh = function(self) {
            ...
            var oldIsVertical = 'undefined' == typeof self.thumbsIsVertical ? false : self.thumbsIsVertical;
            self.thumbsIsVertical = self._isPgvertical && window.innerWidth >= Panda.minDesktopWidth;
            if (self.thumbsIsVertical) {
                self.$thumbs.hasClass('owl-carousel') && self.$thumbs.trigger('destroy.owl.carousel').removeClass('owl-carousel');
                ...
                self.$thumbs.hasClass('owl-carousel') || self.$thumbs.addClass('owl-carousel').owlCarousel($.extend(true, self.isQuickview ? {
        ...
        var recalcDetailsHeight = function() {
            var self = this;
            self.$wrapper.find('.product-details').css('height', window.innerWidth > 767 ? self.$wrapper.find('.product-gallery')[0].clientHeight : '');
        }
    ...
    Panda.initProductSinglePage = (function() {
        ...
        function openFullImage(e) {
            ...
            if ($images.length) {
                var images = $images.map(function() {
                    ...
                    currentIndex = carousel ? ((carousel.current() - carousel.clones().length / 2 + images.length) % images.length) : ($product.find('.product-gallery > *').index());
        ...
        function initWishlistAction(e) {
            ...
            setTimeout(function() {
                $this.removeClass('load-more-overlay loading').html('<i class="p-icon-heart-fill"></i> BROWSE WISHLIST').addClass('added').attr('title', 'Browse wishlist').attr('href', 'wishlist.html');
            }, 500);
        }
    ...
    Panda.slider = (function() {
        ...
        Slider.prototype.init = function($el, options) {
            ...
            $el.on('initialize.owl.carousel', onInitialize).on('initialized.owl.carousel', onInitialized).on('translated.owl.carousel', onTranslated);
            $el.hasClass('animation-slider') && $el.on('initialized.owl.carousel', onSliderInitialized).on('resized.owl.carousel', onSliderResized).on('translate.owl.carousel', onSliderTranslate).on('translated.owl.carousel', onSliderTranslated);
    ...
    Panda.quantityInput = (function() {
        ...
        QuantityInput.prototype.init = function($el) {
            ...
            self.$minus = $el.prev().on('mousedown', function(e) {
                e.preventDefault();
                self.startDecrease();
            ...
            self.$plus = $el.next().on('mousedown', function(e) {
                e.preventDefault();
                self.startIncrease();
            }).on('touchstart', function(e) {
                if (e.cancelable) {
            ...
            Panda.$body.on('mouseup', self.stop).on('touchend', self.stop).on('touchcancel', self.stop);
        }
    ...
    Panda.Menu = {
        ...
        initMenu: function() {
            $('.menu li').each(function() {
                if (this.lastElementChild && (this.lastElementChild.tagName === 'UL' || this.lastElementChild.classList.contains('megamenu'))) {
        ...
        return {
            ...
            open: function(options, callback) {
                ...
                if (!settings.isPurchased) {
                    settings.detailTemplate = Panda.parseTemplate((null != settings.count ? settings.priceQuantityTemplate : settings.priceTemplate), settings)
                } else {
                    settings.detailTemplate = Panda.parseTemplate(settings.purchasedTemplate, settings)
                }
    ...
    Panda.Shop = {
        ...
        initProductType: function(type) {
            if (type === 'slideup') {
                ...
                $(Panda.byClass('product-slideup-content')).on('mouseenter touchstart', function(e) {
                    var $this = $(this),
                        hidden_height = $this.find('.product-hide-details').outerHeight(true);
                ...
                if (!$this.hasClass('toolbox-sort')) {
                    ...
                    if ($selectMenu.is($target.parent())) {
                        ...
                        if ($menuItem.hasClass('active')) {
                            $('.select-items').show();
                            $('<a href="#" class="select-item">' + $target.text() + '<i class="p-icon-times"></i></a>').insertBefore('.select-items .filter-clean').hide().fadeIn().data('link', $menuItem);
                        } else {
        ...
        initProductsCartAction: function() {
            Panda.$body.on('click', '.off-canvas .cart-toggle', function(e) {
                e.preventDefault();
                ...
                if ($compareCol.length > 0) {
                    ...
                    $product.hasClass('product-single') || Panda.Minipopup.open({
                        message: 'Successfully Added',
                        productClass: 'product-mini',
        ...
        initProductsLoginAction: function() {
            Panda.$body.on('click', '.login-toggle', function(e) {
                e.preventDefault();
        ...
        initWishlistButton: function(selector) {
            Panda.$body.on('click', selector, function(e) {
                ...
                setTimeout(function() {
                    $this.removeClass('load-more-overlay loading').find('i').toggleClass('p-icon-heart-solid').toggleClass('p-icon-heart-fill');
                    if ($this.hasClass('added')) {
    ...
    Panda.init = function() {
        ...
        Panda.stickyContent('.sticky-header', {
            top: false
        });
